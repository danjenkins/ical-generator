		...
		var cal = {
			...
			methods: {
				...
				addEvent: function(e) {
					...
					if(e.notification && typeof e.notification === 'object') {
						if(!e.notification.trigger) {
							throw 'event.notification.trigger is empty.';
						}
						if(!e.notification.action) {
							throw 'event.notification.action is empty.';
						}

						_event.notification = {
							action: e.notification.action,
							trigger: e.notification.trigger
						};
					}
				...
				generate: function() {
					...
					cal.events.forEach(function(e) {
						...
						if(e.notification) {
							g += 'BEGIN:VALARM\n';
							g += 'ACTION:' + e.notification.action.toUpperCase() + '\n';
							g += 'TRIGGER:' + e.notification.trigger.toUpperCase() + '\n';
							g += 'END:VALARM\n';
						}
